...
| `sprig-session-incremental-render` | `t` | Redraw only the changed tail of a review buffer on a structural event, keeping the unchanged block prefix in place (nil forces a full redraw each time) |
...
The review buffer (`sprig-session-mode.el`) has its own suite in `sprig-session-mode-tests.el`, which loads `magit-section`. Point the load path at wherever it is installed (locally, the vendored `.deps/` used for development):
