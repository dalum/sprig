...
Below the last message is the **state line**, which says outright whether anything is still going on: `▷ sent, awaiting reply` once your message is away but before the agent has answered, `▶ working…` while its turn streams in, `✓ turn over` once it lands (what it cost is in the header), `✗ turn failed` when it did not, and `● idle` for replayed history (which carries no turn of its own). It sits at the bottom because that is where you are reading when a turn is coming in, and the side bar carries a rule in the same colour, so the gutter ends the turn as plainly as the line does. A turn being over is the thing you wait on, so the buffer states it rather than leaving you to notice that nothing has moved. The line also carries the context in use, since it sits where you are reading: the latest turn's prompt size in tokens and its share of the model's window (`sprig-context-window-tokens`, auto-widening to `sprig-context-window-tiers` for a long-context session), so you can watch the window fill.
...
| `sprig-context-window-tokens` | `200000` | Baseline window the state line's context percentage measures against; a turn using more auto-widens to the tier that fits, so the percentage never exceeds 100 |
| `sprig-context-window-tiers` | `(200000 1000000)` | Known window sizes the context percentage auto-fits to when a turn exceeds the baseline |
