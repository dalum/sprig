...
- A subagent's own steps are not shown individually. The `Agent` row narrates what it is doing while it runs (`▸ Explore: Reading note.txt`) and carries its report when it lands, but the tool calls it made along the way are not rendered as a nested transcript. They are on disk, under `<project>/<session-id>/subagents/agent-*.jsonl`, should that become worth reading.
