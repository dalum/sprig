...
| `c` | Transient, listing every verb: `c c` compose & send (steering a running turn), `c q` compose & queue for after this turn, `c y` / `c n` answer the agent's last prose question yes / no, `c p` compose in plan mode, `c r` resend last turn, `c i` interrupt, `c z` compact the context (`C-u c z` steers the summary), and `c k` / `c C` / `c x` for reject / commit / run |
