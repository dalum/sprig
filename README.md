...
- A subagent's work is not shown: a `Task` call renders as a tool call that starts and returns, with nothing in between, since the transcript reader takes the main thread and skips the sidechains the subagents run on.
