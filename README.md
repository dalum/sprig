...
5. `d` opens the **changeset review**: `d d` for the uncommitted changes, `d m` for the whole branch against main; the top-level `w` says or switches which tree those read. Navigate the changed files, `c c` to comment on a line or a marked region, `c p` to publish every comment to the agent in one turn, and `Q` to quiz yourself on the change before you sign it off. See [Changeset review](#changeset-review) and [Quiz yourself](#quiz-yourself-q).
...
| `c` | Steer the session at point, the session buffer's `c` transient without leaving the list: `c c` compose & send, `c y` / `c n` answer yes / no, `c p` plan mode, `c r` independent review (subagent), `c l` resend, `c i` interrupt, `c z` compact, `c b` a side question (writes no log), `c s` a canned status report the same way, `c q` / `c Q` queue / drop, `c o` open & connect, `c d` disconnect |
...
A long conversation opens on its **last 500 blocks** (`sprig-session-render-window`), and the rest stand as one `⋯ 6,974 earlier blocks not drawn` heading that `RET` pages in a window at a time. Nothing is hidden from the model or the log, only left undrawn, because drawing is what costs: magit-section keeps two markers per section and Emacs adjusts every marker in a buffer on every insertion, so the cost of drawing a block grows with the blocks already drawn. On an 8,000-block session that is the difference between opening in 0.5s and waiting 3s, and it gets worse the longer the session runs. The window is measured once, against the history the buffer is opened or re-read with, and held afterwards, so a turn landing extends the buffer downwards rather than folding away the top of what you are reading. Set it to `nil` to draw everything.

Every tool call folds to its one-line heading, so a long turn reads as a list of what the agent did rather than as pages of diff; `TAB` opens the change you want to review. Set `sprig-session-expand-diffs` to `t` to have diff-bearing tools render open instead. A `TodoWrite` heading carries the plan's progress (`2/5 done`) and unfolds to the checklist itself, each item marked done, in progress, or pending, rather than to a bare tool result. A CLI that instead drives its plan through the granular `TaskCreate`/`TaskUpdate` tools gets the same checklist: those calls fold into one running `Tasks` list rather than showing as their own rows, and a fresh snapshot appears wherever the plan next moved. Tools an MCP server adds need no configuration here: they are wired up on the CLI side (an `.mcp.json` in the project, say), and Sprig renders them like any other tool. The wire name `mcp__server__tool` reads as `server:tool` in the heading, and the summary beside it is taken from whatever the call's input carries, so a tool Sprig has never heard of still says what it was asked for. Permission prompts name it the same way.
...
| `w` | Say or switch which tree the `d` review reads: `w w` / `w c` report, `w d` adopts the detected tree, `w a` asks the agent, `w m` sets by hand, `w r` resets (see [Changeset review](#changeset-review)) |
| `a` | Transient for the agent's structured dialog: `a a` answer, `a r` take the recommended, `a s` skip |
| `c` | Transient, listing every verb: `c c` compose & send (steering a running turn), `c q` compose & queue for after this turn, `c Q` drop the queued messages, `c y` / `c n` answer the agent's last prose question yes / no, `c p` compose in plan mode, `c r` an independent review of the latest changes (the agent spawns a subagent to critique them cold, then addresses its findings; marked sections narrow it), `c l` resend last turn, `c i` interrupt (anything queued then goes), `c z` compact the context (`C-u c z` steers the summary; queued behind a running turn, since a compaction is its own turn), `c b` a side question that leaves the turn and the log alone (see [Side questions](#side-questions)), `c s` a canned status report over the same channel, and `c k` / `c C` / `c x` for reject / commit / run |
...
`c c` opens a compose buffer (`C-c C-c` sends, `C-c C-k` cancels); any marked sections are attached to the message as context, and the first send starts or resumes the session. What is attached is named on a folded `▸ Regarding:` line above the text, the first section's own heading and a count of the rest, and `C-c TAB` unfolds it to exactly what goes out. Sending unmarks those sections, since they have gone with the message; a section you mark while writing is not part of it and stays marked. `c p` sends the turn in plan mode (the agent plans rather than acts), switched over the session's control channel. The mode is **sticky**, the way Claude Code's own is: a plain `c c` afterwards carries on in plan mode rather than dropping out of it, so steering a plan stays a plan. You leave plan mode deliberately, by approving an `ExitPlanMode` plan or with the `P` transient (`P a` back to auto, `P e` accept-edits), which is also how you set the mode by hand at any time. The header shows the permission mode while it is not the normal one, and the mode line carries it too (`[plan]`, `[acceptEdits]`, ...).
...
**Every redraw holds your place.** Re-reading the diff (`g`) keeps the line under point and the files you had open, so a refresh after the agent has worked is a re-read rather than a fresh start. The line is found again by its own text rather than by buffer offset or line number, the same way a draft comment is re-anchored, which is what makes it survive both the diff above it growing and the file around it gaining lines. Where that text now appears twice, the nearest to the old number wins. If the agent has unwritten the very line you were reading, point falls back to roughly where it was. Filing a comment, taking one back, and publishing redraw the same way, so writing a comment mid-diff leaves you where you were rather than at the top, in every window showing the review.

**The code is syntax-highlighted and the change lives in the gutter.** Reviewing is mostly reading code, so the code carries the colours you read it in normally, in the file's own major mode, and whether a line was added or removed is said by the line-number columns and the `+`/`-` marker instead of by painting the whole line. A removed line colours its old-side number, an added one its new-side number, and the marker stays so colour is never the only signal. The colours are computed from the *whole file*, not the hunk: the diff is read a second time at full context (`git diff -U999999`, the same scope over the same transport, one more round trip per refresh), each side of each file is fontified as one text, and the hunk's lines are sliced out by their numbers. So a hunk that starts in the middle of a docstring is painted as the docstring it is in, which a hunk fontified alone cannot know, and whose closing delimiter would flip the colours of everything after it. The per-hunk fontification stays as the fallback, taken when a file runs past `sprig-review-fontify-file-lines` on either side (10,000 by default; nil skips the full-context read entirely), when that read fails, and for any hunk the two reads disagree on, since a tree that moved between them must not paint one version's lines with another's colours. Results are memoised either way, so re-rendering after every comment stays cheap. `sprig-review-fontify-code` turns it all off, which puts the colour back on the whole line.
...
**`e` writes it yourself.** Some feedback is quicker to write than to describe. `e` opens the staging buffer on the code you are looking at, in that file's own major mode, so you edit it with the highlighting and the keys you write that language in. It takes as little as you have pointed at: the region when one is active, else the hunk you marked with `SPC` (marking a chunk is naming it, so it outranks whatever line point is resting on), else **the one line point is on**, which is the grain most hand-authored feedback wants. It widens to the whole hunk only where there is no line to take: on the `@@` heading, on a file heading, or on a removed line, which is not in the file and so has no bytes to edit. Whatever it takes, it is seeded with the *new* side, exactly as it stands on disk, so your edit replaces exactly that, and `C-c C-c` files it as a draft next to your comments (see [Authoring by hand](#authoring-by-hand)). It goes out with the rest of the review on `c p`, not before, and `c e` on it re-opens the staging buffer seeded with what you wrote last time. Because a draft is local, you can write one before the session has ever been started.
...
**What it diffs against.** Four scopes, and the summary line at the top always says which one it is showing, next to the file count and the total stat. Which *tree* they read lives under the top-level `w` transient, which says it, states the tracked directories (`w c`), and can point the scopes at another tree; the `d` popup itself names the tree in force as its heading, so a glance already answers the question.
...
| `w` | Not a scope, and not under `d`: names the tree the four above read, and switches it (detected, asked of the agent, or by hand); `w c` states the tracked directories | `git rev-parse` |
...
**`d p` reviews one branch of a stack.** When you branch off a branch off main, `d m` shows the whole stack, which is everything the branches below yours did as well as yours. `d p` narrows it to what this branch adds.

Git records no parent branch, so it is read off the log the way you would read it in magit: walk back along first-parent history and take the first commit carrying a branch other than this one. On a stack each branch sits on the last, so that is the branch immediately below. Where a commit carries both `foo` and `origin/foo` the local name wins, since it is what you would type and it diffs without a fetch.

It is inference, not a record, and it says so when it fails. A parent that has been merged and deleted leaves its commits unlabelled, so the walk carries past it to whatever is next; a branch taken straight off main answers `main`, making `d p` the same view as `d m`; and with nothing below at all it tells you and asks for a base instead.

**`w w` says where you are.** Every `d` diffs the session's configured working directory, resolved to its git top level. That is not always where the session is, and there are two ways to learn otherwise. The CLI names its own working directory once, on the `init` event, and the two part company when the directory is unset (the CLI runs in the login dir), when it is written with a `~` that only the far end expands, or when the session was re-homed under it. The agent's `Bash` calls say when it moved: a `git worktree add <path>` or a `cd <path>` names the directory in its own command line, which Sprig already parses on the way past. `w w` names the tree, the branch checked out in it, and, when that tree is a linked worktree, the checkout it was added from. When the session is in a *different* tree from the one being reviewed it names that one and its branch too, which is the case the report exists for: a review is then reading a tree the agent is not working in.

**`w` switches the tree too.** Knowing the review reads the wrong tree is only half the job, so `w` is a transient of its own, top-level in the session buffer beside `d`: `w w` is the report above, `w c` the unconditional one below, and the rest point every review verb at another tree. `w d` adopts the detected one, the tree the report would have named, which is the common case in one key: the agent added itself a worktree, and the review should read it. `w a` asks the agent itself, over the same side-question fork `T a` retitles with, so the question sees the whole conversation, writes no log and opens no turn; the answer is proposed in the minibuffer for you to accept or edit before anything is set, since an agent can be wrong about its own shell. `w m` names a tree by hand, with the repository's own worktrees (`git worktree list`) as completion, and `w r` goes back to the session's own directory. Whatever sets it, the tree is checked to be a git tree first, the switch is **review-only** (the session itself stays homed where it is, and a fresh conversation still starts in the session's own directory), and it is buffer-local to the session, so two sessions review two trees without a `setq` between them. The `d` transient's heading names the tree in force, marked `(set with w)` while an override stands, and reopening an already-open review after a switch re-reads it for the new tree the way a scope change does.

**A move is reported as a move.** Reading it from a `Bash` call needs nothing from the agent and adds nothing to the log, which is why it is read there rather than asked for: an instruction to announce its worktree would be forgotten after a compaction, and a stale answer that looks authoritative is worse than none. But it is still an observation with a time on it. Sprig sees the agent go and cannot see the CLI put the shell back afterwards, so `w w` says *the agent was last seen moving into* that tree, naming the verb it read (`git worktree add`, `cd`), rather than claiming it is there now. It never decides which tree a review reads on its own: promoting the detected tree to the one reviewed is a deliberate gesture, one key in the transient (`w d`), never an inference. It is dropped in favour of the CLI's own report when the path no longer names a git tree, which is what a worktree since removed looks like. A path with a space in it, or one carrying a `$` or a backtick the shell would have expanded, is not read at all: half a path is worse than no path.
...
**`w c` states the tracked directories outright.** `w w` is selective: it mentions them only when they land in a different git tree, so that the case worth noticing is not buried under a line printed every time. When the tracked values themselves are what you want, `w c` gives both whether or not they agree, each labelled with where it came from, and names the reviewed directory alongside when it differs from the reported one *as written*. `~/proj` against `/home/me/proj` is one place spelled two ways, and that is exactly the difference that makes agreement look like disagreement. It touches git not at all. The reported value arrives on the CLI's `init` event, so a buffer that has only replayed history has nothing to report until it connects, and `w c` says so rather than answering with a blank.
...
**`d` on a review you already have open shows it as you left it**, and reads nothing: the same folds, the same place, the same drafts. Getting back to a review should not cost you your position in it, and there is already a key that means re-read, which is `g`. Only a fresh buffer, or a `d` that changes the scope (`d m` when you were on `d d`), goes to git.
...
**It aims at what is consequential, and how many it asks scales with the change.** Three questions is a thorough pass over a forty-line change and a spot check on a two-thousand-line one, and passing a spot check is worth knowing you have not done. So `sprig-quiz-questions` is a range, `(3 . 10)` by default, and the ceiling grows with the changed lines (`sprig-quiz-lines-per-question`, 100).

The ceiling is a cap and never a quota, because line count is a poor measure of how much there is to understand: a thousand-line rename holds one idea and a forty-line change to a lock ordering holds several. The generator is told outright that asking fewer is a right answer, and that a large mechanical change may honestly deserve one question, and it takes that seriously: given a 150-file rename and permission for three, it asks one, about whether the rename is complete outside the changeset. So the ceiling only binds where a change genuinely holds that many distinct things, which is where it should not bind, and a generous one costs nothing. Question count costs no extra round trips either, since each fork answers the whole set in one call.

**A long worksheet is answered in the sittings you have.** `C-c C-c` hands in only the questions you have actually written under, and leaves the blanks alone: a question is spent the moment its answers appear, so handing in what you wrote must not spend what you did not. Run it again later for the rest. An answer freezes once handed in, since it is the record of what you actually knew at the time.

An earlier cut weighted the questions towards files the review recorded you had never unfolded. That is gone, and it was wrong three times over. The signal only knows the review buffer, so it cannot tell that you read the file in your editor, in the transcript's inline diff, or wrote it yourself. Skipping is often correct, so aiming there aims at the generated file and the lockfile, which is the reverse of spending attention where it matters. And it is Goodhart: the moment the quiz targets unopened files you start unfolding files to dodge them, corrupting the only attention signal the review had. It bought nothing either, since three hundred lines you waved through fail their questions whether or not the selector knew you waved them through.
...
**Send back what it finds out about the code.** The four outcomes are not symmetric. You wrong and the cold reader right is your gap and nobody else's business. But you right and the cold reader wrong says the code misleads, and both of you wrong says the intent is not recoverable from what is written. Those are defects, located precisely, and the exchange that found them is the evidence.

`C-c C-f` on such a question flags it and opens a note for what it showed, which is the part the agent cannot work out for itself: the question and the answers say where the gap is, you say what it means. `C-c C-p` sends the flagged set back to the session in one turn, composed as a set the way `c p` publishes a review's drafts, and carrying the whole exchange because the note alone cannot say what the gap was. The author's own answer is left out, since the session being sent to already has it.

The covering instruction names the two cases and asks the agent which each one was, because the failure mode is an agent that politely explains a gap the code should have closed. Fix it, do not narrate it.
...
| `sprig-review-fontify-file-lines` | `10000` | Largest file, in lines per side, fontified whole from the review's full-context re-read, so a construct opening above a hunk still colours it right; a longer file falls back to per-hunk fontification. Nil skips the full-context read |
...
| `sprig-session-render-window` | `500` | Trailing blocks a session buffer draws on open; the rest stand as one heading that `RET` pages in a window at a time (nil draws the whole history) |
...
- v0.18.0, written against `claude` 2.1.x. The protocol round-trip (streaming, multi-turn memory, session resume, plan-mode switch) is verified against the real CLI; the Elisp itself has had light exercise, so expect a rough edge or two.
