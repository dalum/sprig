...
| `c` | Steer the session at point, the review buffer's `c` transient without leaving the list: `c c` compose & send, `c y` / `c n` answer yes / no, `c p` plan mode, `c r` independent review (subagent), `c l` resend, `c i` interrupt, `c z` compact, `c b` a side question (writes no log), `c q` / `c Q` queue / drop, `c o` open & connect, `c d` disconnect |
