...
**It aims at what is consequential, and how many it asks scales with the change.** Three questions is a thorough pass over a forty-line change and a spot check on a two-thousand-line one, and passing a spot check is worth knowing you have not done. So `sprig-quiz-questions` is a range, `(3 . 10)` by default, and the ceiling grows with the changed lines (`sprig-quiz-lines-per-question`, 200).
