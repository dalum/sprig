...
- **Next:** live streaming cost. The tee re-renders the whole buffer per event, which is O(conversation) per token; a throttle or an in-place text fast path is needed before long live sessions feel smooth. Then the mark-and-instruction verbs (`c c`, `k`, accept, commit, `x`) and the `c` transient, reading the plist under point.
