...
- `c Q` drop the queued messages, leaving the turn to run; `c u` takes back just the one under point. Deliberately not folded into `c i`, which sends them: a queued message is the next thing, not the rest of this thing, so stopping the turn does not unmake it, and interrupting with one queued is the useful "stop, do this instead". That leaves `c Q` (or `c u`) as the only way to take a queued message back, which it has to be anyway: nothing was sent, so there is nothing to steer or interrupt, and without it a queue would be a thing you could start and never stop. A steer has no such sibling: it is already on the wire, so it cannot be unsent, and `c i` (which stops the whole turn) is the only way to take back mid-turn input. Stopping the turn and meaning it is then `c Q c i`, two gestures that each say one thing, rather than a compound verb that says two.
...
- **Review.** The change lands as a hunk in the working-tree diff, renders as your attributed change, and is now something you can ask the agent to critique. Author, courier, review, in one loop.

**The agent is a courier that cannot tamper.** A live probe against CLI 2.1.224 settled the mechanism as an *override*, not a verify-and-reject. A `can_use_tool` allow response may carry an `updatedInput` that *replaces* the tool's arguments; the probe confirmed the CLI honours an `updatedInput` whose `old_string` / `new_string` the agent never proposed, writing the substituted bytes instead. So apply stages the human's `(file, old_string, new_string)` and asks the agent to make one `Edit`; when that edit surfaces as a permission request, `sprig--maybe-courier` allows it and swaps in the staged strings through `updatedInput`, keeping only the agent's resolved absolute path. The agent supplies no content, so there is nothing to verify and nothing to reject: it triggers the write, Sprig writes the bytes. This needs no special mode, only that the edit reaches a permission prompt, which it does in every mode except the ones that auto-approve edits (`acceptEdits`, `bypassPermissions`); apply refuses in those rather than let the agent's placeholder bytes run. The staged pair is consumed on use, so one stage sanctions exactly one write. If the `old_string` has drifted off disk the `Edit` simply fails and the staged edit can be re-seeded, so drift is safe, not silent.
...
- **Marks and verbs.** `SPC` / `m` mark; a verb acts on the marked set or the section at point. `c` transient: `c c` steer-or-send, `c q` queue, `c Q` drop the queue, `c u` unqueue the message at point, `c p` plan, `c r` independent review, `c l` resend, `c i` interrupt. `k` reject, `x` run, `C` commit, `a` accept, `RET` visit (over TRAMP when remote), `t` retitle. `s n` new conversation, `s f` fork (`--resume --fork-session`).
...
- **Authoring by hand (single region).** `e` is a transient with three ways to seed a local `*sprig-stage*` buffer, opened in the file's own major mode: `e e` the hunk at point (straight from the model), `e f` a file and optional region hint you name, and `e s` the region the agent suggests from the conversation it is already in (with an optional nudge, since it already knows the task). The two agent-read routes ask for one `Read` and seed from its result when the turn ends (its bytes reconstructed from the `cat -n` output are the on-disk `old_string` anchor); `e s` learns the file from the read itself, so no file is named up front. You rewrite the buffer and `C-c C-c` couriers it: apply stages the human's `(file, old_string, new_string)` on `sprig--courier` and asks the agent for one `Edit`; `sprig--maybe-courier` allows that write at its permission prompt and overrides its bytes via `updatedInput` (probe-confirmed against CLI 2.1.224), so the human's bytes land and the agent supplies no content. No mode switch is needed; apply refuses only when the session auto-approves edits. The staged pair is consumed on use, so it sanctions exactly one write.
...
- **Hand-authoring, remaining:** the multi-file **change set** (staging regions from several files at once, then the interleaved prose chunks of the literate-programming lens). Single-region staging with the override courier is shipped (above).
