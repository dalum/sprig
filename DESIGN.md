...
**The agent is a courier that cannot tamper.** The apply verb is a narrow carve-out in the hard block, and a live probe against CLI 2.1.224 settled the mechanism as an *override*, not a verify-and-reject. A `can_use_tool` allow response may carry an `updatedInput` that *replaces* the tool's arguments; the probe confirmed the CLI honours an `updatedInput` whose `old_string` / `new_string` the agent never proposed, writing the substituted bytes instead. So the apply verb stages the human's `(file, old_string, new_string)` and asks the agent to make one `Edit`; the role-gated handler sees the pending staged edit for that file, allows the call, and swaps in the staged strings through `updatedInput`, keeping only the agent's resolved absolute path. The agent supplies no content, so there is nothing to verify and nothing to reject: it triggers the write, Sprig writes the bytes. This is the honest form of the guarantee the hard block gives the author side. The staged pair is consumed on use, so one stage sanctions exactly one write; every other edit stays denied. If the `old_string` has drifted off disk the `Edit` simply fails and the staged edit can be re-seeded, so drift is safe, not silent.
...
- **Navigator staging (single region).** `e` is a transient with three ways to seed a local `*sprig-stage*` buffer, opened in the file's own major mode: `e e` the hunk at point (straight from the model), `e f` a file and optional region hint you name, and `e s` a task you describe that the agent scopes to a file and region for you. The two agent-read routes ask for one `Read` and seed from its result when the turn ends (its bytes reconstructed from the `cat -n` output are the on-disk `old_string` anchor); `e s` learns the file from the read itself. You rewrite the buffer and `C-c C-c` couriers it: apply stages the human's `(file, old_string, new_string)` on `sprig--courier` and asks the agent for one `Edit`; the role-gated handler allows that write and overrides its bytes via `updatedInput` (probe-confirmed against CLI 2.1.224), so the human's bytes land and the agent supplies no content. The staged pair is consumed on use, so it sanctions exactly one write.
