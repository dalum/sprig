...
- **Done (2026-07-17):** `c x` steers rather than sends. Asking the agent to run a command is usually a correction to the turn you are watching, so waiting the turn out to ask defeats it; it now goes down the steer path, which already falls back to a plain send when no turn is running.
