...
- **Next:** `c p` plan mode (needs CLI plan-mode plumbing), finer `x` granularity (a code block inside prose, not just a tool command), model/dir header actions (only meaningful across a reconnect). Optional: incremental section append for the large-history structural render.
