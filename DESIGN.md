...
- **Next:** `c p` plan mode (needs the CLI plan-mode plumbing), `RET` visit-file, finer `x` granularity (a code block inside prose, not just a tool command), and the header made actionable (retitle, change dir/model via transients). Optional: incremental section append for the large-history structural render.
