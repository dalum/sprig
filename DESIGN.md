...
- **Next:** tee the transport into a review buffer (an attached buffer that `sprig--handle` forwards events to) with an entry command, then the mark-and-instruction verbs (`c c`, `k`, accept, commit, `x`) and the `c` transient, reading the plist under point. Open design point: a review buffer attached mid-session only sees events from attach onward, since history lives as rendered Markdown text, not events; making the event log the store (DESIGN's store-vs-view split) is what makes full replay literal.
